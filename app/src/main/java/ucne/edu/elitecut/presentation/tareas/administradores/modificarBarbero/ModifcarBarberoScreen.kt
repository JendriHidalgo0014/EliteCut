        ...
        Box(modifier = Modifier.padding(padding).fillMaxSize()) {
            if (state.isLoading && state.nombre.isEmpty()) {
                ...
                Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState())) {
                    ...
                    Text("Información del Barbero", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onBackground,
                        fontWeight = FontWeight.Bold, modifier = Modifier.padding(horizontal = 16.dp))
                    Spacer(modifier = Modifier.height(12.dp))
                    BarberoFormFields(
                        data = BarberoFormData(nombre = state.nombre, edad = state.edad, telefono = state.telefono, especialidad = state.especialidad, fotoUrl = state.fotoUrl),
                        callbacks = BarberoFormCallbacks(
                            onNombreChange = { onEvent(ModificarBarberoUiEvent.OnNombreChange(it)) },
                            onEdadChange = { onEvent(ModificarBarberoUiEvent.OnEdadChange(it)) },
                            onTelefonoChange = { onEvent(ModificarBarberoUiEvent.OnTelefonoChange(it)) },
                            onEspecialidadChange = { onEvent(ModificarBarberoUiEvent.OnEspecialidadChange(it)) },
                            onFotoUrlChange = { onEvent(ModificarBarberoUiEvent.OnFotoUrlChange(it)) }
                        ),
                    ...
                    Text("Galería de Estilos de Corte", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onBackground,
                        fontWeight = FontWeight.Bold, modifier = Modifier.padding(horizontal = 16.dp))
                    Spacer(modifier = Modifier.height(4.dp))
                    Text("Modifica las fotos de los estilos que realiza este barbero", style = MaterialTheme.typography.bodySmall,
